...
import { buildPayroll, perDayDivisor } from "@/lib/payroll"
import { formatMoney, formatDateLong } from "@/lib/format"
import { exportCSV, printReport, printPayslips } from "@/lib/export"

export default function PayrollTable({ summary, settings, period }) {
  const [search, setSearch] = useState("")
  const [sort, setSort] = useState({ field: "id", dir: "asc" })

  const { rows: allRows, totals, divisor, excludedCount } = useMemo(
  ...
  const handlePrint = () =>
    printReport({
      ...
      rows,
      totals,
      note: `Per-day rate = salary ÷ ${divisor} (${settings.perDayBasis === "working" ? "working days in period" : settings.perDayBasis}). Overtime ${settings.otMethod === "flat" ? `paid at ${formatMoney(settings.otHourlyRate, c)}/hour` : `= salary ÷ (${divisor} days × ${settings.standardHoursPerDay} hours) × OT hours`}. ${settings.deductAbsent ? "Absent days deducted at one day's pay." : "Absent days not deducted."}`,
    })
  ...
  return (
    <div>
      ...
      <div className="mb-3 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-xs text-emerald-800 dark:border-emerald-900/50 dark:bg-emerald-900/20 dark:text-emerald-300">
        <span className="font-semibold uppercase tracking-wide">Assumptions</span>
        ...
        <span>Absent deduction <b>{settings.deductAbsent ? "on" : "off"}</b></span>
