      ...
      <table style="margin-top:14px">
        ...
        <tbody>
          <tr><td>Basic Salary</td><td class="num">${formatMoney(r.salary, c)}</td>
              <td>Absent (${r.unpaidAbsentDays ?? r.absentDays}d unpaid${r.paidLeaveDays ? `, ${r.paidLeaveDays}d paid leave` : ""})</td><td class="num">${formatMoney(r.absentDeduction, c)}</td></tr>
          <tr><td>Overtime (${r.approvedOtHours.toFixed(1)}h approved)</td><td class="num">${formatMoney(r.otPay, c)}</td>
              <td>Half day (${r.halfDays})</td><td class="num">${formatMoney(r.halfDayDeduction, c)}</td></tr>
          <tr><td>Bonus</td><td class="num">${formatMoney(r.bonus, c)}</td>
              <td>Late (${r.lateDays} → ${r.lateDeductionDays}d)</td><td class="num">${formatMoney(r.lateDeduction, c)}</td></tr>
          <tr><td></td><td class="num"></td>
              <td>Penalty</td><td class="num">${formatMoney(r.penalty, c)}</td></tr>
          <tr><td></td><td class="num"></td>
              <td>Advance recovered</td><td class="num">${formatMoney(r.advance || 0, c)}</td></tr>
        </tbody>
        <tfoot>
          <tr><td>Gross Earnings</td><td class="num">${formatMoney(r.salary + r.otPay + r.bonus, c)}</td>
              <td>Total Deductions</td><td class="num">${formatMoney(r.totalDeductions + r.penalty + (r.advance || 0), c)}</td></tr>
        </tfoot>
      </table>
      <table style="margin-top:14px">
